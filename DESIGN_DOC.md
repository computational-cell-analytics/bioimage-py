...
- **The `subprocess` backend is the slurm dress rehearsal.** It implements the entire distributed protocol — temp folder, cloudpickle `payload.pkl`, generated per-task work-lists, per-block persistence into per-task files (a `progress/<task>.log` done-log + length-framed `results/<task>` records, appended after each block with the result written *before* the done-line so a crash leaves at most a harmless re-runnable tail), a final `.success` sentinel, result collection (dedup by block id, merges an original run with a resume), precise per-block failure reporting with a preserved temp folder + failed `block_ids`, a block-counting progress bar, and both `block_ids` re-run and `resume_from` (resume the incomplete tasks of a preserved run). All of this lives in a shared `_DistributedRunner` base; `SubprocessRunner` overrides only `_launch_and_wait` (a local subprocess per task, capped at `num_workers`, accepting a `task_ids` subset for resume). The worker entry point is `python -m bioimage_py.runner._harness <tmp> <task_id> <attempt>`.
...
- **Manifest + reattach.** Both distributed backends write a versioned manifest with an ordered attempt history. Each attempt keeps its task ids, status, timestamps, concurrency, logs, outcomes, and Slurm job id when applicable. Resume appends an attempt and does not replace earlier records. `reattach(tmp_folder)` selects the latest submitted Slurm attempt, reconstructs the partition, and resumes polling. Persisted terminal outcomes can still produce a structured failure after Slurm removes the job from accounting. On `KeyboardInterrupt`, the job continues to run so the user can reattach.
- **Failure diagnostics.** `RunnerError` keeps precise `failed_block_ids` and the preserved temp folder. It also exposes one immutable `TaskFailure` per failed distributed task. Subprocess workers persist exit status, elapsed time, stdout, stderr, and tracebacks. Slurm performs a separate final accounting query for state, exit status, elapsed time, memory, and node details. Missing cluster accounting values remain empty and do not change task success.
