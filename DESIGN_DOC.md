...
The `evaluation` package ports `elf.evaluation` so the metrics are parallel/distributed rather than single-process. Everything rests on one primitive — the **contingency table**, the sparse overlap counts between two segmentations. It is additive across blocks with no halo (blocks partition the volume disjointly), so it is a single-pass reduction exactly like `stats`: each block returns its `(label_a, label_b, count)` rows through the return-value channel and the main process group-sums them into one `ContingencyTable` (counting per block is delegated to `bioimage_cpp.utils.segmentation_overlap`). All metrics except foreground `dice_score` are then cheap, single-process reductions of that table; the expensive per-voxel pass is the table build, which runs across `local`/`subprocess`/`slurm` for free. Each metric has a low-level `*_scores(table, …)` form (operating on a pre-built table — for reuse across metrics, or for tables built distributedly / with resume) and a high-level `(segmentation, groundtruth, …)` wrapper.

Two ignore mechanisms are kept, as in elf: VI/rand/cremi/object-VI take `ignore_seg`/`ignore_gt` and drop those voxels via `ContingencyTable.drop_ignore` (a post-merge pair filter, equivalent to excluding the voxels before counting); matching/mSA/SBD take a single `ignore_label` (default 0) and remove that object's row/column at the score-matrix / per-object stage. Note `drop_ignore` deliberately uses an **OR** combine for the two-sided case (drop where the label is ignored in *either* segmentation), which diverges from elf's AND there — single-sided ignore matches elf.

### Flagged limitations / future work

- **Matching is the scaling wall, not the table.** `matching` / `mean_segmentation_accuracy` densify the table into an `(n_seg, n_gt)` overlap matrix and run Hungarian assignment (`scipy.optimize.linear_sum_assignment`): `O(n_obj²)` memory and `O(n_obj³)` time — in the number of *objects*, not voxels. Fine for typical counts, but it can blow up for volumes with very many objects. A sparse `scipy.sparse.csgraph.min_weight_full_bipartite_matching` over only the non-zero overlaps would lift this; the table build itself stays voxel-scale and parallel.
...
`segmentation.stitching` reconciles a tile-wise segmentation — each tile segmented independently with a halo, so the same object gets a different id in every tile it spans — into one consistent labeling. It is the generic identity-stitching layer for block-wise segmenters such as micro-sam's block-wise APG (see `BLOCK_WISE_TILED_3D_APG.md`), so it has to be *scalable* (never pull the volume into the orchestrator) and *deterministic* (identical output for any worker count and completion order).

- **An instance-correspondence graph, not a RAG.** elf built a region adjacency graph over the assembled tile cores and applied the halo evidence only to pairs that also touch in the cores, so a correspondence whose cores are eroded or shifted at the seam was silently dropped (a P1 reproduced by micro-sam). Here the graph is compact: one node per tile-local object that contributes to a core (the ids the core-assembly stage returns), one edge per object pair with non-zero overlap in a shared halo face (`Blocking.get_local_overlaps`, the intersection of the two clipped haloed tiles), regardless of core contact. Halo-only objects are not nodes, so their evidence is dropped (a known limitation, e.g. a halo-only "bridge" between two core objects). Every voxel-scale phase — tile segmentation into the block store, core assembly, overlap counting, the final `relabel` — is a `runner.run`; only the graph (O(#objects) nodes, O(#overlapping pairs) edges) is built and solved in-process, from per-face `(K, 8)` uint64 rows `[label_a, label_b, count, size_a, size_b, axis, block_a, block_b]` (sizes measured within the face so both directed fractions are available; ids stay uint64, the statistics go float64 before any product; canonicalised and reduced with `lexsort` + `reduceat`, so the result is independent of block order).
- **Symmetric, support-aware merge probabilities.** `overlap_metric` turns `(count, |A|, |B|)` into a merge probability `p` (geometric mean `count / sqrt(|A||B|)` by default; IoU, Dice, min, max = overlap coefficient); `compute_edge_costs(1 - p, beta)` keeps `beta` as the merge threshold (`cost > 0 ⇔ p > beta`). `min_overlap` drops correspondences without absolute support. The rows carry `count` and `axis`, so size weighting (`edge_sizes`) and per-axis calibration (`edge_populations`) can be added later without a format change.
- **Competing correspondences.** Two objects of one tile that both attract the same neighbour object get a soft repulsive edge (`competition_disaffinity`, default 0.8): merging both beats keeping the stronger one iff the *weaker* match's `p` exceeds the disaffinity (several agreeing neighbours add up). Under the geometric mean a cleanly halved object has `p ≈ 0.71` per half, so one contradicting neighbour cannot override a tile-local split (keep-one) while two agreeing neighbours can; the old RAG default 0.9 would need three and is effectively a hard constraint under this metric. Plain negative edges suffice — for repulsive costs the multicut and lifted-multicut objectives coincide.
- **Solve.** `connected_components(edge_mask = cost > 0)`; components without a non-attractive edge between their own nodes are merged outright (their multicut optimum — cutting any edge only adds cost), the remaining ones are solved together in one `multicut_kernighan_lin` sub-problem (a monotone node compaction keeps the edge list valid for `from_unique_edges`). `_solve_correspondence_graph(n_nodes, uv, costs) -> node_labels` is the explicit seam where the block-wise multicut can be substituted.
- **Two-phase API and the block store.** `stitch_segmentation` = fill a per-tile *block store* + `stitch_block_segmentations(store, ...)`. The store has shape `block_store_shape(shape, tile_shape, tile_overlap)` = `(n_tiles, *haloed_tile_shape)`; slot `block_id` holds the haloed tile's labels at its leading corner (tile-local ids, any integer dtype, one chunk per slot so concurrent writes never share a chunk). Externally produced tile segmentations — persistent GPU workers that cannot be cloudpickled as a callback — use the second half directly. Ids are offset by `block_id * prod(haloed_tile_shape)` (`+1` for every id when there is no background, so no object keeps id 0); the assembly stage validates the tile-local id range against this factor. Offset ids are sparse, so the final node labeling is applied as a dict through `relabel` (as `label` does). The store is in-memory only for local execution into an in-memory output and a temp zarr under `tmp_root` otherwise; it is removed on success and preserved on failure (the `RunnerError` carries its path as a note, so the segmented tiles can be stitched with `stitch_block_segmentations`).
- **No direct path, no `block_ids` / `resume_from`.** `tile_shape` is semantic, so there is no whole-array fast path (a single-tile configuration yields the consecutively relabeled segmentation-function result); the stitchers are multi-stage ops, and the two-phase API is their resumability mechanism.

### Flagged limitations / future work

- **The in-process solve is the scaling wall.** Graph construction and the multicut are O(#objects) in the orchestrator; for volumes with very many objects the block-wise / distributed multicut (a separate work package) replaces `_solve_correspondence_graph`. Likewise the `{offset_id: final_id}` dict is cloudpickled into every distributed `relabel` task payload.
- **Seam composition.** Cores are copied as they are: identities are resolved, but a poor prediction near a tile boundary stays visible as a seam, and a core the tile missed is not filled from the neighbour's halo. A separate compositor (distance-/confidence-weighted voting over the halo) after identity resolution, plus a per-instance confidence channel for the edge costs, is the natural follow-up.
- **Face neighbours only.** Evidence is measured across the `2 * ndim` face neighbours; `get_local_overlaps` also handles edge/corner pairs and `get_block_ids_overlapping_bounding_box` enumerates them, so diagonal evidence is a small addition if it turns out to matter.
